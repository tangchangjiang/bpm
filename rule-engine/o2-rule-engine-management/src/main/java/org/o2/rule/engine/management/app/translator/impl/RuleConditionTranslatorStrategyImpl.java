...
public class RuleConditionTranslatorStrategyImpl implements RuleConditionTranslatorStrategy {
    ...
    @Override
    public String translate(Rule rule, String code, String conditionCode, List<RuleMiniConditionParameterDTO> parameters) {
        if (!ruleConditionTranslatorsMap.containsKey(code)) {
            throw new CommonException("there is no condition code: " + code);
        }
        return ruleConditionTranslatorsMap.get(code).translator(rule, conditionCode, parameters);
    }
}
