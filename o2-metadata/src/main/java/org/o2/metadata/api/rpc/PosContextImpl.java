...
public class PosContextImpl implements IPosContext {
    ...
    @Override
    public String posCacheKey(final String posCode, final Long tenantId) {
        if (tenantId == null) {
            return MetadataConstants.PosCache.posCacheKey(0, posCode);
        }
        return MetadataConstants.PosCache.posCacheKey(tenantId, posCode);
    }

    @Override
    public boolean isPosExpressLimit(final String posCode, Long tenantId) {
        final Boolean result = this.redisCacheClient.boundSetOps(MetadataConstants.PosCache.EXPRESS_LIMIT_COLLECTION).isMember(posCacheKey(posCode, tenantId));
        return result != null && result;
    }

    @Override
    public boolean isPosPickUpLimit(final String posCode, Long tenantId) {
        final Boolean result = this.redisCacheClient.boundSetOps(MetadataConstants.PosCache.PICK_UP_LIMIT_COLLECTION).isMember(posCacheKey(posCode, tenantId));
        return result != null && result;
    }
